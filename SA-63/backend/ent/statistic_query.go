...
type StatisticQuery struct {
	...
	withContagious *ContagiousQuery
...
func (sq *StatisticQuery) QueryEmployee() *EmployeeQuery {
...
// QueryContagious chains the current query on the contagious edge.
func (sq *StatisticQuery) QueryContagious() *ContagiousQuery {
	query := &ContagiousQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StatisticQuery) QueryCarrier() *CarrierQuery {
	query := &CarrierQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StatisticQuery) QueryPatient() *PatientQuery {
...
func (sq *StatisticQuery) WithEmployee(opts ...func(*EmployeeQuery)) *StatisticQuery {
	query := &EmployeeQuery{config: sq.config}
...
//  WithContagious tells the query-builder to eager-loads the nodes that are connected to
// the "contagious" edge. The optional arguments used to configure the query builder of the edge.
func (sq *StatisticQuery) WithContagious(opts ...func(*ContagiousQuery)) *StatisticQuery {
	query := &ContagiousQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withContagious = query
	return sq
}

//  WithCarrier tells the query-builder to eager-loads the nodes that are connected to
// the "carrier" edge. The optional arguments used to configure the query builder of the edge.
func (sq *StatisticQuery) WithCarrier(opts ...func(*CarrierQuery)) *StatisticQuery {
	query := &CarrierQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StatisticQuery) WithPatient(opts ...func(*PatientQuery)) *StatisticQuery {
	query := &PatientQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StatisticQuery) sqlAll(ctx context.Context) ([]*Statistic, error) {
	var (
		...
		loadedTypes = [5]bool{
			sq.withEmployee != nil,
			sq.withContagious != nil,
			sq.withCarrier != nil,
			sq.withPatient != nil,
			sq.withArea != nil,
		}
	)
	if sq.withEmployee != nil || sq.withContagious != nil || sq.withCarrier != nil || sq.withPatient != nil || sq.withArea != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withEmployee; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].employee_statistic; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_statistic" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := sq.withContagious; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Statistic)
		for i := range nodes {
			if fk := nodes[i].contagious_statistic; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(contagious.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "contagious_statistic" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Contagious = n
			}
		}
	}
	...
	if query := sq.withCarrier; query != nil {
		...
		nodeids := make(map[int][]*Statistic)
		for i := range nodes {
			if fk := nodes[i].carrier_statistic; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "carrier_statistic" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Carrier = n
			}
		}
	}
	...
	if query := sq.withPatient; query != nil {
		...
		nodeids := make(map[int][]*Statistic)
		for i := range nodes {
			if fk := nodes[i].patient_statistic; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_statistic" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := sq.withArea; query != nil {
		...
		nodeids := make(map[int][]*Statistic)
		for i := range nodes {
			if fk := nodes[i].area_statistic; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "area_statistic" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Area = n
			}
		}
	}
