...
// The ContagiousFunc type is an adapter to allow the use of ordinary
// function as Contagious mutator.
type ContagiousFunc func(context.Context, *ent.ContagiousMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ContagiousFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ContagiousMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ContagiousMutation", m)
	}
	return f(ctx, mv)
}
