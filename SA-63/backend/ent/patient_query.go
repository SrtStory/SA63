...
func (pq *PatientQuery) QueryStatistic() *StatisticQuery {
...
func (pq *PatientQuery) WithStatistic(opts ...func(*StatisticQuery)) *PatientQuery {
	query := &StatisticQuery{config: pq.config}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withStatistic; query != nil {
		...
		query.Where(predicate.Statistic(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.StatisticColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.patient_statistic
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patient_statistic" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_statistic" returned %v for node %v`, *fk, n.ID)
			}
