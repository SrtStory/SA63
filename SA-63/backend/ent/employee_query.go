...
func (eq *EmployeeQuery) QueryStatistic() *StatisticQuery {
	query := &StatisticQuery{config: eq.config}
...
func (eq *EmployeeQuery) WithStatistic(opts ...func(*StatisticQuery)) *EmployeeQuery {
	query := &StatisticQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withStatistic = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withStatistic; query != nil {
		...
		query.Where(predicate.Statistic(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.StatisticColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.employee_statistic
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "employee_statistic" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_statistic" returned %v for node %v`, *fk, n.ID)
			}
