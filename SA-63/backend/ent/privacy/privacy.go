...
// The ContagiousQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type ContagiousQueryRuleFunc func(context.Context, *ent.ContagiousQuery) error

// EvalQuery return f(ctx, q).
func (f ContagiousQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ContagiousQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.ContagiousQuery", q)
}

// The ContagiousMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ContagiousMutationRuleFunc func(context.Context, *ent.ContagiousMutation) error

// EvalMutation calls f(ctx, m).
func (f ContagiousMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.ContagiousMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.ContagiousMutation", m)
}
